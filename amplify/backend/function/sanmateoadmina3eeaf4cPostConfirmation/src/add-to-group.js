...
const addAttribute = async ({laboratoryId, UserPoolId, Username}) => {
  ...
  try {
    ...
    console.log(`[ADD-TO-USER] failed to add ${Username} laboratoryId ${laboratoryId}`, e);
  }
}
/**
...
exports.handler = async event => {
  ...
  if(groupName) {
    await addToGroup({GroupName: groupName, UserPoolId: event.userPoolId, Username: event.userName,});
