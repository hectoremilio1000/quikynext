...
function Results({ signOut, user }) {
  ...
  const getOrdens = async (filter) => {
    const limit = 5;
    const listORDENS = await API.graphql(
      graphqlOperation(queries.listORDENS, {
        limit,
        nextToken,
        filter: {
          ...filter,
          ...
          // or: [
          //   { status: { eq: Statusorder.VERIFIED } },
          //   { status: { eq: Statusorder.SENT } },
          // ],
        },
      })
    );
  ...
  const fetchOrdenes = async () => {
    if (!loading) {
      ...
      if (groupName === GROUPS.DOCTOR) {
        const listDOCTORS = await API.graphql(
          graphqlOperation(queries.listDOCTORS, {
        ...
        setObserver(doctor);
        const ordens =
          (await getOrdens({ ...filter, doctorID: { eq: doctor.id } })) || [];
        const listPACIENTES = await API.graphql(
          graphqlOperation(queries.listPACIENTES, {
            filter: {
              or: (ordens || []).map((orden) => ({
                id: { eq: orden.pacienteID },
              })),
            },
          })
        );
        const patients = listPACIENTES?.data?.listPACIENTES?.items.reduce(
          (all, patient) => {
            all[patient.id] = patient;
            return all;
          },
          {}
        );
        setOrdens(
          ordens.map((orden) => ({
            ...orden,
            contestant: patients[orden.pacienteID],
          }))
        );
      }
      ...
      if (groupName === GROUPS.PATIENT) {
        ...
        setObserver(patient);
        const ordens =
          (await getOrdens({ ...filter, pacienteID: { eq: patient.id } })) ||
          [];
        const listDOCTORS = await API.graphql(
          graphqlOperation(queries.listDOCTORS, {
            filter: {
              or: (ordens || []).map((orden) => ({
                id: { eq: orden.doctorID },
              })),
            },
          })
        );
        const doctors = listDOCTORS?.data?.listDOCTORS?.items.reduce(
          (all, doctor) => {
            all[doctor.id] = doctor;
            return all;
          },
          {}
        );
        setOrdens(
          ordens.map((orden) => ({
            ...orden,
            contestant: doctors[orden.doctorID],
          }))
        );
      }
      setLoading(false);
  ...
  useEffect(() => {
    if (
      !atuhLoading &&
      (groupName !== GROUPS.DOCTOR || groupName !== GROUPS.PATIENT)
    ) {
  ...
  const isResultReady = (status) => {
    switch (status) {
      case Statusorder.CREATED:
        return false;
      case Statusorder.PUBLISHED:
        return false;
      case Statusorder.VERIFIED:
        return true;
  ...
  return (
    <div
      className="bannerSanmateo afterBanner posRelative"
        ...
        <div className="blockElement flex flex-row">
          <div className="basis-1/4">
            <div className="bg-gray-200 p-4 m-4 text-center rounded-md">
              ...
              <button
                className="bg-gray-300 text-red-600 rounded-md mt-8 py-2 px-4 my-2"
                onClick={signOut}
              >
                Sign Out
              </button>
                    ...
                    <div className="select-none cursor-pointer bg-gray-200 rounded-md flex flex-1 items-center p-4  transition duration-500 ease-in-out transform hover:-translate-y-1 hover:shadow-lg">
                      <div className="flex-1 pl-1 mr-16">
                        <div className="font-medium mb-4">
                          {orden?.contestant?.nombres} {orden.fechaOrden}{" "}
                          {orden.horaOrden}
                        </div>
                        <Grid container spacing={2}>
                          ...
                          <Grid item xs={12} md={6}>
                            ...
                            <div className="text-gray-600 text-sm">
                              <b>Sucursal Muestra: </b>
                              {orden.sucursalMuestra}
                            </div>
                            <div className="text-gray-600 text-sm">
                              <b>Peso: </b>
                              {orden.peso}
                      ...
                      {/* {isResultReady(orden.status) && ( */}
                      <div
                        onClick={() => handleOpen(orden)}
                        className="text-center flex justify-center items-center rounded-md w-20 h-10 bg-gray-300 text-blue-600 text-xs  p-4"
                      >
                        Resultado
                      </div>
                      {/* )} */}
